  ...
  return (
    <div className="flex flex-col md:flex-row min-h-screen bg-slate-50 pt-20 md:pt-24 font-sans text-slate-800">
      ...
      <div className="flex-1 w-full px-4 py-6 md:px-12 md:py-10 overflow-y-auto">
        ...
        {/* MAGIC TRICK HERE: 
            We changed "flex-col" to "flex-col-reverse". 
            This tells the phone to put the BOTTOM item (Terminal) on TOP!
            "lg:flex-row" keeps them side-by-side exactly like before on computers.
        */}
        <div className="flex flex-col-reverse lg:flex-row gap-8 lg:gap-10 items-start w-full">
          
          {/* LEFT COLUMN ON DESKTOP (Tables) - Now appears SECOND on mobile */}
          <div className="flex-1 w-full bg-white p-4 md:p-8 rounded-xl shadow-sm border border-slate-100 overflow-hidden">
            ...
            <div className="flex flex-row overflow-x-auto border-b border-slate-200 mb-6 gap-2 hide-scrollbar">
              {['input', 'output', 'errors'].map((tab) => (
                <button 
                  ...
                  className={`whitespace-nowrap pb-3 px-3 md:px-4 text-[14px] md:text-[15px] font-bold capitalize transition-all duration-200 border-b-2 ${
                    activeTab === tab 
                ...
                <table className="w-full text-left border-collapse min-w-[600px]">
                  <thead>
                    <tr className="bg-slate-50 border-b-2 border-slate-200">
                      <th className="py-3 px-4 md:py-4 md:px-5 text-[#0070c0] font-bold text-[13px] md:text-[14px]">Name</th>
                      <th className="py-3 px-4 md:py-4 md:px-5 text-[#0070c0] font-bold text-[13px] md:text-[14px]">Type</th>
                      <th className="py-3 px-4 md:py-4 md:px-5 text-[#0070c0] font-bold text-[13px] md:text-[14px]">Description</th>
                      <th className="py-3 px-4 md:py-4 md:px-5 text-[#0070c0] font-bold text-[13px] md:text-[14px] text-center">Mandatory</th>
                    </tr>
                  </thead>
                  <tbody>
                    {apiData.inputData.map((row, index) => (
                      <tr key={index} className="hover:bg-slate-50 transition-colors border-b border-slate-200 last:border-none">
                        <td className="py-3 px-4 md:py-4 md:px-5 text-[13px] md:text-[14px] font-semibold text-slate-800">{row.name}</td>
                        <td className="py-3 px-4 md:py-4 md:px-5 text-[13px] md:text-[14px] text-slate-500 font-mono bg-slate-50/50">{row.type}</td>
                        <td className="py-3 px-4 md:py-4 md:px-5 text-[13px] md:text-[14px] text-slate-600 leading-relaxed">{row.desc}</td>
                        <td className="py-3 px-4 md:py-4 md:px-5 text-[13px] md:text-[14px] font-bold text-center">
                          <span className={`inline-flex items-center px-2.5 py-1 rounded-full text-[11px] md:text-xs font-bold ${
            ...
            {activeTab === 'errors' && (
              <div className="overflow-x-auto rounded-lg border border-slate-200 w-full">
                <table className="w-full text-left border-collapse min-w-[500px]">
                  <thead>
                    <tr className="bg-slate-50 border-b-2 border-slate-200">
                      <th className="py-3 px-4 md:py-4 md:px-5 text-[#0070c0] font-bold text-[13px] md:text-[14px]">Error Code</th>
                      <th className="py-3 px-4 md:py-4 md:px-5 text-[#0070c0] font-bold text-[13px] md:text-[14px]">Error Name</th>
                      <th className="py-3 px-4 md:py-4 md:px-5 text-[#0070c0] font-bold text-[13px] md:text-[14px]">Error Logs</th>
                    </tr>
                  </thead>
                  <tbody>
                    {apiData.errorData.map((row, index) => (
                      <tr key={index} className="hover:bg-slate-50 transition-colors border-b border-slate-200 last:border-none">
                        <td className="py-3 px-4 md:py-4 md:px-5 text-[13px] md:text-[14px] font-bold text-red-600 font-mono">{row.code}</td>
                        <td className="py-3 px-4 md:py-4 md:px-5 text-[13px] md:text-[14px] font-medium text-slate-700">{row.name}</td>
                        <td className="py-3 px-4 md:py-4 md:px-5 text-[13px] md:text-[14px] text-slate-500">{row.logs}</td>
                      </tr>
                    ))}
                  </tbody>
                </table>
          ...
          {/* RIGHT COLUMN ON DESKTOP (Terminal & Buttons) - Now appears FIRST on mobile */}
